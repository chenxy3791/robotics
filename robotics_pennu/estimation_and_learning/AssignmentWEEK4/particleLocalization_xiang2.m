...
function myPose = particleLocalization_xiang2(ranges, scanAngles, map, param)
...
for j = 2:N    
    ...
    for i = 1:M
        ...
        for k = 1:N_occ
            ...
            if map(i_y_occ(k), i_x_occ(k)) > 1.0
    ...
    N_eff = floor(1./sum(W_norm.*W_norm));
    
    %if(N_eff < 0.8*M)
    
        %fprintf(1,'N_eff = %d; Resampling...\n',N_eff);
